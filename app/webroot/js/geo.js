...
function GEOprocess(position) {
  ...
  //document.getElementById('geo').innerHTML = 'Latitude: ' + position.coords.latitude + ' Longitude: ' + position.coords.longitude;
	...
	GEOajax(get_url()+"classifieds/getuserlocation?accuracy=" + position.coords.accuracy + "&latlng=" + position.coords.latitude + "," + position.coords.longitude +"&altitude="+position.coords.altitude+"&altitude_accuracy="+position.coords.altitudeAccuracy+"&heading="+position.coords.heading+"&speed="+position.coords.speed+"");
}
...
function updatePage() {
 if (xmlHttp.readyState == 4) {
  ...
  document.getElementById("region_city").value = response.trim();
 }
}
